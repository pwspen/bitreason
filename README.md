...
Current performance of two solvers, neural network and PySR (symbolic regression library), on the full task set is below. The two plots are generated by the code in examples [nn_sweep](examples/nn_sweep.py) and [pysr_sweep](examples/pysr_sweep.py), respectively.
