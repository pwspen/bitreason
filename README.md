...
Inputs are randomly generated then passed through a function to produce the output, so just by defining a function with signature `Callable[[list[int]] -> list[int]]`, you get access to 2^length input-output pairs (65536 currently). You can easily adjust the number of train and test samples per task.

Current tasks (easy to add your own): `[not, and, or, xor, sum, shift, flip, tile, count, separate, divisors, gcd]`

(See the exact functions in [task_list.py](src/minireason/task_list.py))

See [the basic use example](examples/basic.py):
...
Current performance of two solvers, neural network and PySR (symbolic regression library), on the full task set is below. The two plots are generated by the code in examples [nn_sweep](examples/nn_sweep.py) and [pysr_sweep](examples/pysr_sweep.py), respectively. The code is identical except for the solver used (solvers live in [solvers.py](src/minireason/solvers.py))
